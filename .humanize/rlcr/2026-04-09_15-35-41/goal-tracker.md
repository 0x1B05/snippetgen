...
### Plan Version: 17 (Updated: Round 5 Review Fixes)
...
| 4-review | Rejected reopening AC-1 around `compose.ops` because the immutable implementation plan in `detail-plan.md` remains the goal-alignment authority for this loop, while the execution-prep doc is a secondary reference with a divergent suite-field example | The implementation, tests, and checked-in suite file already satisfy the accepted Round 4 contract under the current `compose.snippets` schema, so reopening `task7`, `task10`, `task11`, and `task12` would create tracker drift rather than close an original-plan gap | No scope change; Round 4 completion stands, the schema-name mismatch is tracked as a queued documentation issue, and the verified task records are restored |
...
| 5 | Re-anchored Round 5 to the review-phase findings without widening scope beyond the accepted first-stage PoC contract | The code review highlighted fidelity and validation gaps in the build path, so the round stayed focused on fixing those defects rather than adding new behavior | No scope change; the fixes close defects in the existing AC-1/AC-4 path rather than extending the PoC |
| 5 | Fixed the review-phase fidelity and validation defects and revalidated the serial build path | The generated harness now propagates configured seeds, the boot path reaches `main`, the scalar-load snippet emits a real misaligned `lw` on RISC-V, repeated snippet sources are deduplicated, unsupported languages are rejected early, and malformed seeds are rejected during suite loading | No scope change; AC-1 through AC-4 remain the same and the queued follow-up list is unchanged |
...
|-------|-----------------|------------------|-----------------|
| The execution-prep doc still shows `compose.ops` while the accepted Round 4 implementation plan, checked-in suite file, and tests use `compose.snippets` | 4-review | The immutable goal and ACs come from `detail-plan.md`, which does not require changing the field name, so this is a documentation-consistency issue rather than a blocker for the completed first-stage PoC | Revisit if the plan/docs are being consolidated or the suite schema is intentionally renamed in a later scope expansion |
...
| AC-1 | task7: Implement `suite_loader.py` to load `scalar_load_legality_poc.yaml`, validate `sequence` mode, and produce a deterministic plan | 3 | 3-review | Codex re-ran `python3 -m unittest tests.test_snippet_loading` and `make dump-plan`; the reviewed loader enforces `future-only` mode rejection, unsupported-target rejection, invalid suite-name rejection, and a deterministic real-suite plan under the accepted Round 4 suite schema |
| AC-2 | task8: Implement `emitter.py` to generate `build/<suite>/generated_suite.c` from the ordered snippet list | 5 | pending | The emitted harness now stays suite-driven, propagates non-default seeds into `env.seed`, and the scalar-load legality path still compiles and orders snippets correctly under `tests.test_build_pipeline` |
| AC-4 | task9: Implement `toolchain.py` to compile runtime/snippet/harness sources, link `test.elf`, and export `test.bin` | 5 | pending | The toolchain now deduplicates repeated snippet source compilation and the serial `python3 generator/cli.py build` plus `make build` path still succeeds after the review fixes |
| AC-1, AC-4 | task10: Create `scalar_load_legality_poc.yaml` and verify the build path generates `test.elf`, `test.bin`, and `build_manifest.json` | 5 | pending | Suite loading now rejects malformed seeds up front, the checked-in suite still emits stable artifacts under `build/scalar_load_legality_poc/`, and the generated ELF now has a boot path that reaches `main` |
| AC-1, AC-2 | task11: Add unit tests for manifest loading, suite loading, and harness emission failure modes | 5 | pending | `tests.test_runtime_surface`, `tests.test_snippet_loading`, and `tests.test_build_pipeline` now pass together (`Ran 28 tests ... OK`), including regressions for real misaligned-load intent, strict seed validation, unsupported languages, duplicate snippet references, and seed propagation |
| AC-3, AC-4 | task12: Review produced artifacts and prune any accidental future-only dependencies | 5 | pending | The Round 5 review fixes are verified locally and the next review-phase pass should confirm that no new future-only or mainline regressions were introduced by the boot, snippet, loader, and toolchain changes |
